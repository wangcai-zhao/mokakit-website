...
export const SITE = {
   ...
   *
   * ⚠️ 里面的「工具总数」是硬编码的，每批新工具上线后要同步改这里 ——
   * site.ts 同时被客户端打包引用，没法直接 import registry 去动态数（会把一堆工具元数据拖进 bundle）。
   * 改动后请顺跑 `node scripts/changelog-check.mjs` 确认新工具都已登记。
   * 当前工具数 240（截至 2026-09-23）。
   */
  description:
    '摩卡工具箱（MokaKit）是 AI 时代的在线工具箱：240 个免费工具，涵盖个税社保、房贷契税、单位换算、图片处理、文本处理、开发辅助与文档脱敏等，打开即用、本地计算不上传数据；同时通过 MCP 协议供 Claude、Cursor 等 AI 助手直接调用。',
